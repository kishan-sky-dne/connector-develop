...
def test_delete_ipaddress_instance():
    delete_ipaddress_obj = DeleteIpAddress(delete_kwargs)
    assert isinstance(delete_ipaddress_obj, DeleteIpAddress)


@patch("connectors.core.utils.rest_api_utility.RestUtility.delete")
def test_delete_ip(delete_mock):
    delete_ipaddress_obj = DeleteIpAddress(delete_kwargs)
    delete_mock.return_value = 204
    expected_delete_status = {"status": "success"}
    assert delete_ipaddress_obj.delete_ip() == expected_delete_status


# Negative case
@patch("connectors.core.utils.rest_api_utility.RestUtility.delete")
def test_delete_ip_negative(delete_mock):
    delete_ipaddress_obj = DeleteIpAddress(delete_kwargs)
