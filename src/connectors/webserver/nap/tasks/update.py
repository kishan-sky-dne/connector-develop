...
def check_final_status(output: list) -> str:
    """
    Method will take data in below format and return response
    :param output:
        output1 = [{1: {"status": "SUCCESS", "error": ""}}, {2: {"status": "SUCCESS", "error": ""}}]
        output2 = [{1: {"status": "SUCCESS", "error": ""}}, {2: {"status": "FAIL", "error": ""}}]
        output3 = [{1: {"status": "FAIL", "error": ""}}, {2: {"status": "FAIL", "error": ""}}]

    :return:
        check_final_status(output1)  # Output: SUCCESS
        check_final_status(output2)  # Output: PARTIAL-SUCCESS
        check_final_status(output3)  # Output: FAIL
    """
    statuses = [item[next(iter(item))]["status"] for item in output]
...
@exception_handler
def update_nap_details(**kwargs: dict) -> dict:
    """
    ...
    Returns:
            {}
    """
    region = kwargs.get("body", {}).get("region", "Ireland").lower()
    logger.info(f"Entering into NEXUS Api module to fetch details for region {region} with kwargs: {kwargs}")
    update_url = (
        f"{base_url.replace('REGION', region_mapper.get(region, 'roi'))}/support/api/order/REQUESTID/v1/voice/"
        f"network/in-flight/resubmit"
    )
    request_id = kwargs.get("body", {}).get("requestId", [])
    ...
    nap_obj = NapService()
    status, output = nap_obj.update_nap_details(**input_kwargs)
    logger.info("Exiting Nexus update module after sending the api response")
    return {"status": check_final_status(output), "orders": output}
